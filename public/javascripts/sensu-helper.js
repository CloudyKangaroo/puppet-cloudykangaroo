...
function handleTicketForm(event) {
  ...
  if (newTicket) {
    var data = {subject: sensuEvent.check + ' - ' + sensuEvent.client.replace('.contegix.mgmt', ''), sensuEvent: encodeURI(JSON.stringify(sensuEvent)), documentation: documentation, recipient: recipient, priority: sensuEvent.status, clientID: sensuEvent.clientid, deviceID: sensuEvent.dev};
    url += 'ticket';
  } else {
